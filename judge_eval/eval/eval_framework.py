...
"""
from judge_eval.configs.models import OPENAI_MODELS
from judge_eval.eval.data_sampler import DataSampler
from judge_eval.eval.judges import LLMJudge
from judge_eval.eval.metrics_computation import MetricsComputation
from judge_eval.eval.metrics_visualization import Visualization
import os
...
class JudgesEval:
  ...
  def __init__(
      self,
      data_path="./datasets/formatted_datasets",
    ...
    self.metrics_computer = MetricsComputation(data_path=cache_dir,
                                               num_runs=num_runs,
                                               cache_dir=cache_dir)
    self.visualizer = Visualization(dataset_id=dataset_id, cache_dir=cache_dir)
  ...
  def _check_templates(self):
    ...
    for template in self.templates:
      ...
      if task != self.dataset_id:
        raise NameError(
            f"prompt template {template} is not for the task {self.dataset_id}!"
        )
      # Check if the template is implemented
      if not os.path.exists(
          f"judge_eval/templates/{self.dataset_id}/{paper}/{template}.py"):
  ...
  def _create_judges(self):
    ...
    return [
        LLMJudge(llm=llm,
                 ...
                 cache_dir=self.cache_dir) for llm in self.models
        for template in self.templates
    ]
  ...
  def evaluate_judges(self):
      ...
      for tdx, template in enumerate(self.templates):
        ...
        for split_id in range(self.num_splits):
          ...
          for run_id in range(num_runs):
            ...
            # Make judgings
            judge_results = llm_judge.make_judging(
                datalist=datalist,
                split_id=split_id,
                run_id=run_id,
        ...
        acc_bias_results = self.metrics_computer.compute_acc_bias(
            judge_results=judge_results_all_splits,
            dataset_id=self.dataset_id,
            llm=llm,
            template=template,
        ...
        self_consist_results = self.metrics_computer.compute_self_consistency(
            dataset_id=self.dataset_id,
            llm=llm,
            template=template,
    ...
    all_results = self.metrics_computer.post_process_results(
        judges_acc_bias_results,
