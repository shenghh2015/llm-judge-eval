...
"""
from judge_eval.configs.models import OPENAI_MODELS, MODEL_SERVICES
...
from judge_eval.utils.utils_judge import remove_redundant, make_judge_result
...
class BaseJudge:
  ...
  def __init__(self, data_path, dataset_id, cache_dir):
    ...
    assert os.path.exists(
        os.path.exists(os.path.join(
            self.data_path,
            self.dataset_id))), "sampled data path does not exist!"
  ...
  def _check_cached_data(self, split_id, run_id, datalist_len):
    ...
    full_cache_dir = os.path.join(self.cache_dir, self.dataset_id,
                                  "judging_results", self.judge_name)
    latest = find_latest(
        os.path.join(
            full_cache_dir,
            f"**.{self.dataset_id}.{self.judge_name}.split_id{split_id}.run_id{run_id}.jsonl"
        ))
...
class LLMJudge(BaseJudge):
  ...
  def __init__(self,
    ...
    assert extract_rule in [
        "combine", "chosen_reject", "reject_chosen"
    ], f"extract_rule must be combine, chosen_reject or reject_chosen!"
    ...
    if self.dataset_id == "summarize":
      assert self.template_name.split(
          '_')[1] == "summarize", "template should be for summarization task!"
    if self.dataset_id == "hhrlhf":
      assert self.template_name.split(
          '_')[1] == "hhrlhf", "template should be for hh-rlhf task!"
    
    if llm in OPENAI_MODELS:
      print("Commercial LLM judge is used!")
      self.api_key = os.environ.get("OPENAI_API_KEY", "")
      self.base_url = MODEL_SERVICES.get(llm, "https://api.openai.com/v1")
      assert self.api_key != "", "ERROR: Please OPENAI_API_KEY!!!!"
    else:
      self.api_key = "EMPTY"
      self.base_url = MODEL_SERVICES.get(llm, "")
      assert self.base_url != "", f"ERROR: Please set base_url for model {llm} !!!!"
      print("Open-sourced LLM judge is used!")

    # llm inference service client
  ...
  def _init_judge(self):
    ...
    print('*' * 20, " Judge Info ", '*' * 20)
    print(
        f"Data: {self.dataset_id} | LLM: {self.llm} | Template: {self.template_name} | Temperature: {self.temperature} | Extract Rule: {self.rule}"
    )
    ...
    print("Judge client is created!")
    
    return f"{llm_name}_{template_name}"  # judge name

  def _multi_thread_processing(self, datalist, num_threads):
    results = [None] * len(datalist)
    for batch_idx in range(0, len(datalist), num_threads):
      start_idx, end_idx = batch_idx * num_threads, min((batch_idx + 1) * num_threads, len(datalist))
      threads = []
      for idx in range(start_idx, end_idx):
        threads.append(
            Thread(target=make_judge_result,
                  args=(datalist[idx], self.template_name, self.llm_client, self.llm, self.temperature, self.max_new_tokens,
                        self.rule, idx, results)))
        threads[-1].start()
      for td in threads:
        td.join()
    return results

  def make_judging(self,
                   datalist,
                   split_id,
                   run_id,
                   num_eval=-1,
    ...
    #   threads.append(
    #       Thread(target=make_judge_result,
    #              args=(data, self.template_name, self.llm_client, self.llm, self.temperature,
    #                    self.rule, idx, results)))
    #   threads[-1].start()
    ...
    #  pool.join()
    # results = sorted(results, key=lambda x: x['id'])
    results = self._multi_thread_processing(datalist, num_threads=num_workers)
    print(f'Number of judging results: {len(results)}')

    # Write the judging results into the file
    new_dir(
        full_cache_dir)  # if it does not exist, create the cache directory.
    output_path = os.path.join(
        full_cache_dir,
        f"{date.today().strftime('%Y_%m_%d')}.{self.dataset_id}.{self.judge_name}.split_id{split_id}.run_id{run_id}.jsonl"
    )
