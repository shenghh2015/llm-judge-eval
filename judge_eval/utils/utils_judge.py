...
def import_template(template_name=''):
  ...
  module = importlib.import_module(f".{template_name}",
                                   package=f"judge_eval.templates.{task}.{paper_name}")
...
def create_comparison_prompt(original_text, response1, response2, prompt):
...
def evaluate_response_pair(
  ...
  prompt = create_comparison_prompt(original_text, response1, response2,
                                    template)
...
def make_judge_result(_data,
  ...
  if mode == 'combine':
    _cached_result_cr, _len_prompt_cr, _len_response_cr, _run_time_cr = evaluate_response_pair(
        prompt, chosen, reject, template, model_client, model, temperature, max_new_tokens)
    _cached_result_rc, _len_prompt_rc, _len_response_rc, _run_time_rc = evaluate_response_pair(
        prompt, reject, chosen, template, model_client, model, temperature, max_new_tokens)
    ...
    _cached_result, _len_prompt, _len_response, _run_time = evaluate_response_pair(
        prompt, chosen, reject, template, model_client, temperature)
  else:
    _cached_result, _len_prompt, _len_response, _run_time = evaluate_response_pair(
        prompt, reject, chosen, template, model_client, temperature)
  ...
  if mode == 'combine':
    ...
    data['chosen_score'], data['reject_score'] = get_rm_scores(
        _cached_result, template)
    ...
    data['reject_score'], data['chosen_score'] = get_rm_scores(
        _cached_result, template)
    data['gpt4_eval'] = format_analysis_result(_cached_result,
                                               reject + '(reject)',
                                               chosen + '(chosen)',
